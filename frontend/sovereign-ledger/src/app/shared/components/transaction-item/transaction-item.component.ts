...
@Component({
  ...
  template: `
    <div (click)="openDetail.emit(transaction())"
         class="flex items-center justify-between p-4 bg-surface-container-lowest rounded-xl shadow-sm hover:bg-surface-container-low transition-all cursor-pointer group active:scale-[0.98]">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-xs text-on-surface-variant uppercase font-bold tracking-tighter">{{ transaction().category }} &bull; {{ transaction().date }}</p>
        </div>
      </div>
      <div class="text-right">
        <p class="font-bold font-headline"
           [class]="transaction().type === 'credit' ? 'text-on-tertiary-fixed-variant' : 'text-on-surface'">
          {{ transaction().type === 'credit' ? '+' : '-' }}{{ transaction().amount | currency:'PHP':'symbol':'1.2-2' }}
        </p>
